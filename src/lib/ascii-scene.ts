...
export class ASCIIAnimation {
	...
	setupEventListeners() {
		...
		this.mousemoveHandler = (e: MouseEvent) => {
			window.userX = Math.floor(
				e.clientX / ((this.fontSize / 2) * scale),
			);
		...
		this.touchstartHandler = (e: TouchEvent) => {
			...
			window.userY = Math.floor(
				e.touches[0].clientY / (this.fontSize * scale),
			);
		};
	...
	initSprites() {
		...
		this.plane = new Sprite(
			this.assets[
				this.noncommercial ? "plane.txt" : "corporate_plane.txt"
			],
			[0, 1, 2, 3],
			this.camera,
		);
		...
		sprite.z = 20;
		sprite.x =
			(this.width / 2) * sprite.z - sprite.width * 1.5 * sprite.z;
		...
		// Farm car
		this.farmCar = new Sprite(
			this.assets["farmCar.txt"],
			[0],
			this.camera,
		);
		this.farmCar.z = 4.5;
		...
		// Car far
		this.carFar = new Sprite(
			this.assets["carfar.txt"],
			[0],
			this.camera,
		);
		this.carFar.z = 4.5;
	...
	handleCowStealing() {
		...
		if (this.cowAsteal === 1 && this.spaceship) {
			if (
				...
				this.spaceship.x +=
					(this.cowAX -
						(this.spaceship.displayX() +
							this.spaceship.width / 2)) /
						10 +
					0.1;
			} else {
	...
	animateObjects(time: number) {
		...
		if (this.plane) {
			...
			this.plane.x =
				this.plane.x <
				(-this.width / 2 - this.plane.width) * this.plane.z
		...
		if (this.car2) {
			...
			this.car2.x =
				this.car2.x <
				(-this.width / 2 - this.car2.width) * this.car2.z
		...
		// Shark
		if (this.shark) {
			this.shark.move(time / 120, [
				Math.sin(time / 1000) * 2,
				0,
			]);
		}
		...
		// Car far
		if (this.carFar) {
			this.carFar.move(time / 60, [
				Math.sin(time / 1000) * 6,
				0,
			]);
		}
